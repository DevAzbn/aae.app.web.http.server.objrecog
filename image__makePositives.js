...
azbn.mdl('fs/tree').walk(argv.from, function(file, stat){
				...
				if(err) {
					...
					for(var j = 0; j < argv.copies; j++) {
						...
						(function(_image, _j){
								...
								_image.write('./data/opencv/education/process/positives/' + argv.type + '/' + new_file, function(_err){
	...
	tasks.push(function(callback) {
		...
		app.saveFile('opencv/education/process/positives/' + argv.type + '.txt', result_files.join('\n'));
